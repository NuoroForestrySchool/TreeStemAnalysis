...
Fusti modello, dati di base [ATTENZIONE: Fusti con anno di nascita recente]
```{sql connection=con}
...
Misure con incongruenze evidenti (ho ipotizzato delle correzioni, sarebbe bene, verificare sulle rotelle)
```{r}
print(dbGetQuery(con, "select Id_fusto, Id_rotella, ripetizione, anno, misura0, misura1 from AdF0 where misura1<>misura0"))
```


Raggi con letture in eccesso
```{sql connection=con}
...
```{r}
irmax <- dbGetQuery(con,"select Id_fusto, Id_rotella, ripetizione, anno, max(ir) from CalcolaIncRaggio group by Id_fusto, Id_rotella, ripetizione")
names(irmax)[length(irmax)] <- "max_ir"
hist(irmax$anno[irmax$ripetizione=='a'], breaks = max(irmax$anno)-min(irmax$anno)+1)
hist(irmax$anno[irmax$ripetizione=='b'], breaks = max(irmax$anno)-min(irmax$anno)+1)

```

OSSERVAZIONE: i due grafici non appaiono molto correlati, evito per ora di confondere le ripetizioni!  
